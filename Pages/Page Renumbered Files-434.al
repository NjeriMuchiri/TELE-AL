OBJECT page 172029 Cheque Transactions Card
{
  OBJECT-PROPERTIES
  {
    Date=02/18/20;
    ...
    ActionList=ACTIONS
    {
      ...
      { 1102760078;2 ;Action    ;
                      ...
                      OnAction=BEGIN
                                 ...
                                 MODIFY;
                                 END;
                                 IF "Cheque Status"<> "Cheque Status"::Approved THEN
                                 ERROR('send for approval first');
      ...
      { 1102760026;1 ;Action    ;
                      ...
                      PromotedCategory=Process;
                      OnAction=BEGIN
                                 // IF "Transaction Type"= "Cheque No" THEN
                                 //   ERROR('send for approval');
  ...
  CODE
  {
      ...
      //SMS
          ...
          IF Vend1.FIND('-') THEN BEGIN
            ...
            IF Amount<>0 THEN BEGIN
              SMSMessage."SMS Message":='Dear' +FORMAT(Vend1.Name)+ 'You have done a Cheque Deposit Transaction of KSHS. '+
              FORMAT(Amount)+' on '+FORMAT(TODAY) + ' ' +FORMAT(TIME)+' to your Account at TELEPOST SACCO SOCIETY Head Office, Please call +254727438688 if in dispute.';
            END;
    ...
    BEGIN
      ...
      //SMS
          ...
          IF Vend1.FIND('-') THEN BEGIN
            ...
            SMSMessage."Sent To Server":=SMSMessage."Sent To Server"::No;
            IF Amount<>0 THEN BEGIN
              SMSMessage."SMS Message":='Dear' +FORMAT(Vend1.Name)+ 'You have deposited a Bankers Cheque Transaction of KSHS. '+
              FORMAT(Amount)+' on '+FORMAT(TODAY) + ' ' +FORMAT(TIME)+'from your Account at TELEPOST SACCO SOCIETY Head office. Please call +254727438688 if in dispute';
            END;
            IF Vend1."MPESA Mobile No" <> '' THEN BEGIN
              // SMSMessage."Telephone No":=Vend1."MPESA Mobile No";
            END ELSE BEGIN
    ...
    BEGIN
      ...
      GenJournalLine.VALIDATE(GenJournalLine."Account No.");
      GenJournalLine."Posting Date":="Transaction Date";
      GenJournalLine.Description:='Fixed depositExcise duty Charges';
      GenJournalLine.VALIDATE(GenJournalLine."Currency Code");
      ...
      IF (TransactionAmount<=0) THEN BEGIN
       ...
       IF Type = 'Withdrawal' THEN BEGIN
          ...
          GenJournalLine.VALIDATE(GenJournalLine."Currency Code");
          GenJournalLine.Amount:=((Amount*2.5)/100);
          GenJournalLine.VALIDATE(GenJournalLine.Amount);
          GenJournalLine."Bal. Account Type":=GenJournalLine."Bal. Account Type"::"G/L Account";
      ...
      GenJournalLine.VALIDATE(GenJournalLine.Amount);
      GenJournalLine."Bal. Account Type":=GenJournalLine."Bal. Account Type"::"Bank Account";
      GenJournalLine."Bal. Account No.":='BNK00013';
      GenJournalLine.VALIDATE(GenJournalLine."Bal. Account No.");
      GenJournalLine."Shortcut Dimension 1 Code":=DActivity;
      GenJournalLine."Shortcut Dimension 2 Code":=DBranch;
      GenJournalLine.VALIDATE(GenJournalLine."Shortcut Dimension 1 Code");
      ...
      {IF Account.GET("Account No") THEN BEGIN
      IF Account."Staff Account"= FALSE THEN BEGIN}
      //MESSAGE('ChargeAmount is %1',ChargeAmount);
      //ERROR('ChargeAmount is %1',ChargeAmount);
      Echarge:=ChargeAmount;
      LineNo:=LineNo+1000;
      //GenJournalLine.RESET;
      //GenJournalLine.SETRANGE("Journal Template Name",'PURCHASES');
      ...
      //SMS
          ...
          IF Vend1.FIND('-') THEN BEGIN
            ...
            SMSMessage."SMS Message":='Dear' +FORMAT(Vend1.Name)+ 'You have done a Cash Deposit transaction of KSHS. '+FORMAT(Amount)+

                                      ' on '+FORMAT(TODAY) + ' ' +FORMAT(TIME)+' to your Account at TELEPOST SACCO SOCIETY Head office. Please call +254727438688 if in dispute';
              END ELSE BEGIN
    ...
    BEGIN
      ...
      IF CurrentTellerAmount-Amount<=TellerTill."Min. Balance" THEN
      MESSAG('You need to add more money from the treasury since your balance has gone below the teller replenishing level.');

      IF ("Transaction Type"='Withdrawal') OR ("Transaction Type"='Encashment') OR ("Transaction Type"='MPESA') THEN BEGIN
      IF (CurrentTellerAmount - Amount) < 0 THEN
      ERROR('You do not have enough money to carry out this transaction.');
      EXIT;
      END;
      MESSAGE('CurrentTellerAmount %1',CurrentTellerAmount);
      IF (TransactionTypes.Type=TransactionTypes.Type::Withdrawal) OR (TransactionTypes.Type=TransactionTypes.Type::Encashment)OR (TransactionTypes.Type=TransactionTypes.Type::"M-pesa") THEN BEGIN
      IF CurrentTellerAmount - Amount>=TellerTill."Maximum Teller Withholding" THEN
      MESSAGE('You need to transfer money back to the treasury since your balance has gone above the teller maximum withholding.');
