OBJECT page 172167 Matured Fixed Deposit List
{
  OBJECT-PROPERTIES
  {
    Date=07/23/20;
  ...
  PROPERTIES
  {
    ...
    SourceTableView=WHERE(Posted=FILTER(No),
                          Fixed=FILTER(Yes));
    PageType=List;
    CardPageID=Credited FD Card;
    ActionList=ACTIONS
    {
      { 1120054019;0 ;ActionContainer;
      ...
      { 1120054015;1 ;Action    ;
                      ...
                      OnAction=BEGIN
                                 IF Posted=TRUE THEN
                                   ERROR('You cannot Revoke an already posted Fixed deposit');
                                 IF "FD No"='' THEN
                                   ERROR('There is nothing to Revoke here');
                                 ...
                                 GenJournalLine."Document No.":='revoked fixed deposit';
                                 GenJournalLine.Description:='revoked Fixed deposit ';
