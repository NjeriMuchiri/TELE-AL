OBJECT page 50028 Loan Appraisal Card
{
  OBJECT-PROPERTIES
  {
    Date=07/18/23;
  ...
  PROPERTIES
  {
    ...
    OnOpenPage=BEGIN
                 ...
                 IF LoansR."Loan Status"=LoansR."Loan Status"::Appraisal THEN

                 //IF loa"Loan Status"="Loan Status":: THEN
    ...
    ActionList=ACTIONS
    {
      ...
      { 1102760045;2 ;Action    ;
                      ...
                      OnAction=BEGIN
                                 ...
                                 IF LoanApp.FIND('-') THEN BEGIN
                                   {IF USERID=LoanApp."Captured By" THEN
                                     ERROR('You cannot appraise this loan since you are the one who captured it');}
                                 ...
                                 IF Register.FIND('-') THEN BEGIN
                                  IF (USERID<>'TELEPOST\GOMINDE') OR (USERID<>'TELEPOST\MNDEKEI')  THEN BEGIN
                                   OffsetDetails.RESET;
      ...
      { 1000000014;2 ;Action    ;
                      ...
                      OnAction=BEGIN
                                 ...
                                 IF LoansRegister6.FINDSET THEN BEGIN
                                   REPEAT
                                 ...
                                 END ELSE BEGIN
                                   "Total Deductions(Salary)":="Total Deductions(Salary)"+LoansRegister6.Repayment; //commected monthly repayment
                                   END;
                                   END;
                                 ...
                                 MESSAGE('%1',"Total Deductions(Salary)");
                                 ...
                                 StandingOrders.SETFILTER(StandingOrders."End Date",'<=%1',TODAY);
                                 IF StandingOrders.FINDSET THEN BEGIN
                                 REPEAT
                                 "Total Deductions(Salary)":="Total Deductions(Salary)"+StandingOrders.Amount;
                                 ...
                                 //new net take
                                 CALCFIELDS("Bridge Amount Release");
                                 "Net take Home":="Two Thirds Basic"-"Total Deductions(Salary)"+"Bridge Amount Release";
                                 Nettakehome:="Two Thirds Basic"-"Total Deductions(Salary)"+"Bridge Amount Release";
                                 END;


                                 IF ("Recovery Mode"="Recovery Mode"::Checkoff) THEN BEGIN
                                 IF "Employer Code"='POSTAL CORP' THEN BEGIN
                                 Nettakehome:=GrossPay-("Third basic"+"Check off Deductions"+1000);
                                 END ELSE BEGIN
                                 Nettakehome:=GrossPay-("Third basic"+"Check off Deductions");
                                 END;
                                 END;
                                 //end of get
                                 {IF LoansR."Recovery Mode"= LoansR."Recovery Mode"::Checkoff THEN BEGIN
      ...
      { 1120054008;2 ;Action    ;
                      ...
                      OnAction=BEGIN
                                 ...
                                 ELSE IF "Repayment Frequency"="Repayment Frequency"::Weekly THEN
                                 RunDate:=CALCDATE('-1W',RunDate)
                                 ELSE IF "Repayment Frequency"="Repayment Frequency"::Monthly THEN BEGIN
                                   EVALUATE(RunDate,'01/01/2019');
      ...
      { 1120054012;2 ;Action    ;
                      Name=updateoffset;
                      CaptionML=ENU=Update Loan offsets;
                      Promoted=Yes;
                      Image=AdjustEntries;
                      PromotedCategory=Process;
                      OnAction=BEGIN
                                 IF CONFIRM('Are you sure you want to update the offset details') THEN BEGIN
                                   LoanOffsetDetails.RESET;
                                   LoanOffsetDetails.SETRANGE(LoanOffsetDetails."Loan No.","Loan  No.");
                                   IF LoanOffsetDetails.FINDSET THEN BEGIN
                                     REPEAT
                                       ScheduleRep6.RESET;
                                       ScheduleRep6.SETRANGE(ScheduleRep6."Loan No.",LoanOffsetDetails."Loan Top Up");
                                       ScheduleRep6.SETFILTER(ScheduleRep6."Repayment Date",'>=%1',TODAY);
                                       IF ScheduleRep6.FINDFIRST THEN BEGIN
                                         LoanOffsetDetails."Interest Top Up":=ScheduleRep6."Monthly Interest";
                                         LoanOffsetDetails."Principle Top Up":=ScheduleRep6."Principal Repayment";
                                         LoanOffsetDetails."Monthly Repayment":=ScheduleRep6."Monthly Repayment";
                                         LoanOffsetDetails.MODIFY;
                                         END;
                                       UNTIL LoanOffsetDetails.NEXT=0;
                                     END;
                                   END;
      ...
      { 3       ;2   ;Action    ;
                      ...
                      PromotedCategory=Process }
      { 1120054010;2 ;Action    ;
                      Name=<Page Loans Guarantee DetailsN>;
                               ...
                               BEGIN
                                 ...
                                 //End allocate batch number
                                   {
                                    ...
                                    SendSms('Guarantors' ,Cust."Mobile Phone No",'You have guaranteed '+ "Client Name" + ' ' + "Loan Product Type" +' of KES. '+FORMAT("Approved Amount")+
                                 ...
                                 IF "Loan Status"="Loan Status"::Rejected THEN BEGIN
                                    ...
                                    SMSMessages.RESET;
                                       ...
                                       //SMSMessages."Sent To Server":=SMSMessages."Sent To Server::No;
                                       SMSMessages."SMS Message":='Your loan application of KSHs.'+FORMAT("Approved Amount")+
                                                                 ' has been rejected. TELEPOST SACCO LTD';
                                       Cust.RESET;
                                       IF Cust.GET("Account No") THEN
                                       SMSMessages."Telephone No" := Cust."Phone No.";
                                       SMSMessages.INSERT;
                                 END;
                                         ...
                                         AuditTrail.FnInsertAuditRecords(EntryNo,USERID,'Loan Appraisal',"Requested Amount",'CREDIT',TODAY,TIME,"Loan  No.","Loan  No.","Client Code",'');
  ...
  CONTROLS
  {
    ...
    { 1000000012;2;Field  ;
                ...
                Editable=false;
                OnValidate=BEGIN
                             //"Loan Principle Repayment":=ScheduleRep."Principal Repayment";
                           END;
                            }
    ...
    { 1000000013;2;Field  ;
                ...
                Editable=FALSE;
                OnValidate=BEGIN
                             //"Loan Interest Repayment":=ScheduleRep."Monthly Interest";
                           END;
                            }
    ...
    { 1102760002;2;Field  ;
                OptionCaptionML=ENU=Transfer To FOSA ,Cheque,EFT,RTGS,Cheque NonMember,FOSA Loans,Individual Cheques>;
    ...
    { 1120054011;2;Field  ;
                SourceExpr="Check off Deductions" }
  ...
  CODE
  {
    VAR
      ...
      OffsetDetails@1000000113 : Record 51516235;
      CheckoffMatrix@1000000114 : Record 51516286;
      ReasonForRejection@1120054000 : Text[80];
      LoanAppraisalSalaryDetails@1120054001 : Record 51516232;
      Basic@1120054002 : Decimal;
      OtherIncome@1120054003 : Decimal;
      LoansRegister6@1120054004 : Record 51516230;
      Charges@1120054005 : Record 51516297;
      ProcessingFee@1120054006 : Decimal;
      SaccoGeneralSetUp@1120054007 : Record 51516257;
      LoanOffsetDetails@1120054008 : Record 51516235;
      Itemsoffset@1120054009 : Decimal;
      GuarantorsDetail@1120054013 : Boolean;
      SalaryEditable@1120054010 : Boolean;
      AuditTrail@1120054016 : Codeunit 51516107;
      Trail@1120054015 : Record 51516655;
      EntryNo@1120054014 : Integer;
      StandingOrders@1120054011 : Record 51516307;
      ScheduleRep6@1120054012 : Record 51516234;
    ...
    BEGIN
      ...
      "Client Code"+' '+"Client Name"+' has been received and is under process '
    ...
    LOCAL PROCEDURE FnGenerateSchedule@1000000008();
    VAR
      NLInterest@1000000000 : Decimal;
      InterestVarianceOnlyNafaka@1000000001 : Decimal;
    BEGIN
      IF "Repayment Frequency"="Repayment Frequency"::Daily THEN
      EVALUATE(InPeriod,'1D')
      ELSE IF "Repayment Frequency"="Repayment Frequency"::Weekly THEN
