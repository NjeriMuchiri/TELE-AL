OBJECT page 172158 Matured  Fixed deposit list
{
  OBJECT-PROPERTIES
  {
    Date=06/03/22;
  ...
  PROPERTIES
  {
    ...
    SourceTableView=WHERE(Fixed=FILTER(Yes),
                          Posted=FILTER(No));
    PageType=List;
    CardPageID=Matured FD Card;
    ...
    ActionList=ACTIONS
    {
      ...
      { 1120054014;1 ;Action    ;
                      ...
                      OnAction=BEGIN
                                 ...
                                 GenJournalLine.Description:='Witholding Tax On Interest Earned';
                                 GenJournalLine.VALIDATE(GenJournalLine."Currency Code");
                                 GenJournalLine.Amount:=-("Interest Earned"*15/100);
                                 GenJournalLine.VALIDATE(GenJournalLine.Amount);
                                 GenJournalLine."Shortcut Dimension 1 Code":=DActivity;
                                 GenJournalLine."Shortcut Dimension 2 Code":=DBranch;
                                 GenJournalLine.VALIDATE(GenJournalLine."Shortcut Dimension 1 Code");
  ...
  CODE
  {
    VAR
      ...
      CreditEnabled@1120054103 : Boolean;
